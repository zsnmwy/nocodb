...
const searchAndReplace = (target) => {
  ...
  const list = dfs(dirPath)

  for(const pkg of ['nc-gui', 'nocodb']) {
    const packageJson = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'packages', pkg, 'package.json'), 'utf8'))
    packageJson.dependencies[nocodbSdkPackage.name] = nocodbSdkPackage.version
    fs.writeFileSync(path.join(__dirname, '..', 'packages', pkg, 'package.json'), JSON.stringify(packageJson, null, 2), 'utf8')
  }
